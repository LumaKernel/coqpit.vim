...
"        some are specialized for Coquille.

let g:coquille#job#supported = 0

if has('job')  " {{{1
  let g:coquille#job#supported = 1

  function! coquille#job#job_start(cmd, opt) abort
    let l:opt = {}
  ...
  function! coquille#job#job_status(job) abort
    return job_status(a:job)
  endfunction

  function! coquille#job#job_stop(job, ...) abort
    if a:0
      call job_stop(job)
    else
  ...
  function! coquille#job#job_setoptions(job, opt) abort
  ...
  function! coquille#job#ch_sendraw(job, raw) abort
    call ch_sendraw(a:job, a:raw)
  endfunction

elseif has('nvim')  " {{{1
  let g:coquille#job#supported = 2
...
elseif " {{{1
  function! coquille#job#job_start() abort
